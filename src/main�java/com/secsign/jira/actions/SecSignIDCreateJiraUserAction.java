...
public class SecSignIDCreateJiraUserAction extends JiraWebActionSupport {
     ...
     */
    private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(SecSignIDCreateJiraUserAction.class);
    ...
    public void doValidation(){
        ...
        logger.debug("doValidation / request parameter map: " + SecSignIDLogger.toString(httpRequest.getParameterMap()));
    ...
    public String doDefault() throws Exception {
        ...
        HttpServletRequest httpRequest = getHttpRequest();
        logger.debug("doExecute / request parameter map: " + SecSignIDLogger.toString(httpRequest.getParameterMap()));
    ...
    public String doExecute() {
        ...
        if ((isAddUser || isSignUp) && ! Action.ERROR.equals(resultingTemplateName)){
            ...
            if(secsignid != null && secsignid.length() > 1){
                ...
                logger.info("secsign id '" + secsignid + "' is (are) assigned to created application user '" + username + "'.");
                
                ApplicationUser appUser = SecSignIDUserManager.getApplicationUser(username);
                String userNameToSet = appUser != null ? appUser.getKey() : username.toLowerCase();
                ...
                secSignIdMappings.put(userNameToSet, secSignIds);
                ...
                passwordLoginAllowed.put(userNameToSet, pwdAllowed);
    ...
    private ApplicationUser createUserOnJIRA6() {
        ...
        try{
            logger.debug("fetch method createUserNoNotification.");
            ...
            if(createUserMethod != null){
                ...
                logger.info("created new application/directory user '" + username + "'.");
            } else {
            ...
            logger.error(ex.getMessage(), ex.getTargetException());
        }
        ...
        if(createdApplicationUser == null){
            logger.info("could not get newly created application user '" + username + "'");
            ...
            if(jiraUsersDefaultGroup != null){
                try {
                    ...
                    try {
                        ...
                        logger.info("could not find class 'com.atlassian.crowd.embedded.api.User'.");
                    }
                    ...
                    if(userClass != null){
                        logger.debug("fetch method addUserToGroup to add object of type of " + String.valueOf(userClass));
                        ...
                        if(addUserToGroupMethod != null){
                            ...
                            logger.info("application/directory user '" + createdUser + "' was put into group 'jira-users'.");
                        }
                    }
                } 
                catch(InvocationTargetException ex){
                ...
                logger.info("could not find group 'jira-users' nor 'jira-software-users'. therefor user '" + createdUserObject + "' will not be added to any group.");
            }
        } else {
            logger.info("created application user '" + username + "' is not put into any JIRA user group because the license is exceeded.");
        }
    ...
    private ApplicationUser createUserOnJIRA7() {
        ...
        try{
            ...
            logger.debug("fetch method createUser using UserDetails as parameter.");
            
            Method createUserMethod = umgr.getClass().getMethod("createUser", userDetailClass);
            createUserMethod.invoke(umgr,  userDetailsObject);
            
        } catch(ClassNotFoundException ex){
            logger.info("could not find class 'com.atlassian.jira.user.UserDetails'.");
        } catch(Exception ex){
            logger.error(ex);
        }*/
            ...
            if(jiraUsersDefaultGroup != null){
                try {
                    ...
                    try {
                        ...
                        logger.info("could not find class 'com.atlassian.jira.user.ApplicationUser'.");
                    }
                    if(userClass == null){
                        userClass = createdUserObjectType;
                    }
                    ...
                    if(userClass != null){
                        ...
                        if(addUserToGroupMethod != null){
                            addUserToGroupMethod.invoke(userUtil, jiraUsersDefaultGroup, createdUserObject);
                            logger.info("application/directory user '" + createdApplicationUser + "' was put into group 'jira-users'.");
                        }
                    }
                } 
                catch(InvocationTargetException ex){
